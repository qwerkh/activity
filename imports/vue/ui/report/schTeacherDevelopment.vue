<!--suppress ALL -->
<template>
    <div class="schTeacherDevelopment-report">
        <a4>
            ...
            <span slot="content" style="margin: 0px !important;">
                      ...
                      <caption>
                          ...
                          <div class="row">
                              <div class="col-md-12  balckOnPrint" style="text-align: center;">
                                 ...
                                 <p style="font-family: 'Khmer OS Moul'">បញ្ជីការអភិវឌ្ឍន៍ធនធានមនុស្ស ប្រចាំត្រីមាសទី {{quarter}} ឆ្នាំ {{year}}</p>
                ...
                <tbody style="margin-bottom: 5px;" v-html="teacherDevelopmentHtml">
...
<script>
    ...
    export default {
                ...
                pickerDateOptions: {
                    shortcuts: [{
                        ...
                        onClick(picker) {
                            ...
                            const end = moment().startOf("year").add(2, "month").endOf("month").toDate();
                            picker.$emit('pick', [start, end]);
                        }
                    }, {
                        text: 'Quarter 2',
                        onClick(picker) {
                            const start = moment().startOf("year").add(3, "month").startOf("month").toDate();
                            const end = moment().startOf("year").add(5, "month").endOf("month").toDate();
                            picker.$emit('pick', [start, end]);
                        }
                    }, {
                        text: 'Quarter 3',
                        onClick(picker) {
                            const start = moment().startOf("year").add(5, "month").startOf("month").toDate();
                            const end = moment().startOf("year").add(8, "month").endOf("month").toDate();
                            picker.$emit('pick', [start, end]);
                        }
                    }, {
                        text: 'Quarter 4',
                        onClick(picker) {
                            const start = moment().startOf("year").add(9, "month").startOf("month").toDate();
                            const end = moment().startOf("year").add(11, "month").endOf("month").toDate();
                            picker.$emit('pick', [start, end]);
                        }
                    }]
                },
        ...
        methods: {
            ...
            fetchDevelopment() {
                let selector = {};
                Meteor.call("queryDevelopmentOption", selector, (err, result) => {
                    if (result) {
            ...
            handleRun() {
                ...
                Meteor.call('schTeacherDevelopmentReport', this.params, this.langConfig, (err, result) => {
                    if (result) {
                        this.teacherDevelopmentHtml = result.teacherDevelopmentHTML;
        ...
        computed: {
            ...
            langConfig() {
                let data = compoLangReport.filter(config => config.lang === this.langSessionReport)[0]['teacherDevelopment'];
                return data;
            }
        },
        components: {
